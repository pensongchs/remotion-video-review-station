...
description: 搭建和运行无视频缓存的 Remotion 视频审片台，通过源文件路径载入单条或多条已渲染视频，按时间点和画面选框记录修改建议，汇总为可直接执行的 Remotion 修改与重新渲染提示词。用户提到视频审片、视频批注、选框修改、批量审片、重新渲染、审片台、预览台，或要求以后生成的视频先统一提修改意见时使用。
---
